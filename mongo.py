...
class MongDb(object):
    ...
    def __init__(self, db_host, db_port, db_name, db_user, db_pass, log=None):
        try:
            self.log = log
            ...
            if db_user and db_pass:
                self.connected = self.db.authenticate(db_user, db_pass)
            else:
                self.connected = True
        except Exception as e:
            self.log.error(
                '{db} {port} {name} {user} {pwd}'.format(db=db_host, port=db_port, name=db_name, user=db_user,
                                                         pwd=db_pass))
